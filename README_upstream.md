...
[![CI (Latest)](https://github.com/randriu/synthesis/actions/workflows/ci-latest.yml/badge.svg?branch=master)](https://github.com/randriu/synthesis/actions/workflows/ci-latest.yml)
[![CI (Stable & Fetched)](https://github.com/randriu/synthesis/actions/workflows/ci-stable.yml/badge.svg?branch=master)](https://github.com/randriu/synthesis/actions/workflows/ci-stable.yml)
...
After synthesis completes, three additional flags trigger optimization of the resulting FSC:

- ``--output-dir PATH``: saves FSC artefacts (JSON/pkl files, ``results.json``) to the given directory.
- ``--dt-conversion``: exports the inductively synthesized FSC (F_I) as decision trees into ``--output-dir``, one tree per memory node, using [dtControl](https://dtcontrol.model.in.tum.de/). Requires ``--output-dir``.
...
All flags can be combined in a single SAYNT run. The following example runs SAYNT on the maze POMDP, exports decision trees for the synthesized FSC, and minimizes the belief FSC:

```shell
python3 -m paynt models/pomdp/maze/slip --fsc-synthesis --storm-pomdp --iterative-storm 900 60 10 --minimize-storm-fsc --output-dir out/maze --dt-conversion
```
