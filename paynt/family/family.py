...
class Family:
    ...
    def size(self):
        return math.prod(
            [self.family.holeNumOptions(hole) for hole in range(self.num_holes)]
        )
    ...
    def size_or_order(self):
        order = int(
            math.fsum(
                [
                    math.log10(self.family.holeNumOptions(hole))
    ...
    def generate_labels(self):
        ...
        for length in range(1, 5):  # Adjust the range for longer sequences if needed
    ...
    def extract_unique_values(self, arr):
        ...
        actions = [
            {
                "observation": get_current_observation(a),
                "type": get_current_type(a),
                "memory": get_current_memory(a),
                "actions": get_current_value(a),
            }
            for a in arr
            if get_current_type(a) == "A"
        ]
        memory = [
            {
                "observation": get_current_observation(a),
                "type": get_current_type(a),
                "memory": get_current_memory(a),
                "next_memory": int(get_current_value(a)[0]),
            }
            for a in arr
            if get_current_type(a) == "M"
        ]
        mixed = [
            {
                "observation": get_current_observation(a),
        ...
        for a in actions:
            ...
            matching_records = [
                m
                for m in memory
                if m["observation"] == a["observation"] and a["memory"] == m["memory"]
            ]
    ...
    def build_edge_dict(self, actions, unique_observations, unique_actions):
        ...
        for a in actions:
            ...
            if key not in edge_dict:
                edge_dict[key] = {
                    "observations": set(),
                    "action": actionsToShortDict[a["actions"][0]],
                }
    ...
    def toGraph(self, route=None):
        ...
        unique_memory_values, unique_observations, unique_actions, actions, memory = (
            self.extract_unique_values(arr)
        )
        ...
        edge_dict, obsToShortDict, actionsToShortDict = self.build_edge_dict(
            actions, unique_observations, unique_actions
        )
        ...
        for (memory, next_memory, action), labels in edge_dict.items():
            ...
            dot.edge(
                str(memory),
                str(next_memory),
                label=f"{observation_label}/{action_label}",
            )
        ...
        with dot.subgraph(name="cluster_legend") as legend:
            ...
            legend.node(
                "legend",
                label=self.generate_legend(obsToShortDict, actionsToShortDict),
                shape="plaintext",
            )
    ...
    def split(self, splitter, suboptions):
        return [
            self.assume_hole_options_copy(splitter, options) for options in suboptions
        ]
    ...
    def pick_random(self):
        hole_options = [
            [random.choice(self.hole_options(hole))] for hole in range(self.num_holes)
        ]
        return self.assume_options_copy(hole_options)
