...
class Synthesizer:
    ...
    def choose_synthesizer(quotient, method, fsc_synthesis=False, storm_control=None):
        ...
        if isinstance(quotient, paynt.quotient.pomdp.PomdpQuotient) and fsc_synthesis:
            return paynt.synthesizer.synthesizer_pomdp.SynthesizerPomdp(
                quotient, method, storm_control
            )
        # FSC synthesis for Dec-POMDPs
        if (
            isinstance(quotient, paynt.quotient.decpomdp.DecPomdpQuotient)
            and fsc_synthesis
        ):
        ...
        if isinstance(quotient, paynt.quotient.mdp_family.MdpFamilyQuotient):
            if method == "onebyone":
                return paynt.synthesizer.synthesizer_onebyone.SynthesizerOneByOne(
                    quotient
                )
            else:
        ...
        if method == "ar_multicore":
            return paynt.synthesizer.synthesizer_multicore_ar.SynthesizerMultiCoreAR(
                quotient
            )
    ...
    def time_limit_reached(self):
        if (
            self.synthesis_timer is not None
            and self.synthesis_timer.time_limit_reached()
        ) or paynt.utils.timer.GlobalTimer.time_limit_reached():
    ...
    def evaluate(self, family=None, prop=None, keep_value_only=False, print_stats=True):
        ...
        if self.export_synthesis_filename_base is not None:
            self.export_evaluation_result(
                evaluations, self.export_synthesis_filename_base
            )
    ...
    def synthesize(
        ...
        if family.constraint_indices is None:
            family.constraint_indices = list(
                range(len(self.quotient.specification.constraints))
            )
        ...
        if self.memory_constraintfunc != None:
            family = self.get_memory_restrained_family(family)
        self.stat.start(family)
        self.synthesize_one(family)
        if (
            self.best_assignment is not None
            and self.best_assignment.size > 1
            and not return_all
        ):
        ...
        if self.best_assignment is not None:
            ...
            converter = FSCtoDTConverter(str(self.best_assignment))
    ...
    def run(
        self, optimum_threshold=None, memory_constraint="none", generated_fsc_route=None
    ):
            ...
            if get_current_type(restricted_family.hole_to_name[hole]) == "M":
                if self.memory_constraintfunc:
                    restricted_family.hole_set_options(
                        hole,
                        self.memory_constraintfunc(
                            restricted_family.hole_to_name[hole],
                            restricted_family.hole_options(hole),
...
def simpleCircle(name, option_labels, max_memory):
    current = get_current_memory(name)
    indices = [i for i, label in enumerate(option_labels) if int(label) == current + 1]
    if not indices:
        indices = [
            i for i, label in enumerate(option_labels) if label == "0" or label == 0
        ]
    return indices
...
def bothWayCircle(name, option_labels, max_memory):
    ...
    if len(indices) == 1:
        if current == 0:
            indices.extend(
                [i for i, label in enumerate(option_labels) if int(label) == max_memory]
            )
        else:
            indices.extend(
                [
...
def bothWayCircleSelfLoop(name, option_labels, max_memory):
    ...
    indices = [
        ...
        or int(label) == current - 1
        or int(label) == current
    ]
    if len(indices) == 2:
        if current == 0:
            indices.extend(
...
def notDecreasingCyclic(name, option_labels, max_memory):
    current = get_current_memory(name)
    indices = [i for i, label in enumerate(option_labels) if int(label) >= current]
...
def notDecreasing(name, option_labels, max_memory):
    current = get_current_memory(name)
    return [i for i, label in enumerate(option_labels) if int(label) >= current]
...
def binaryTreeSelfLoop(name, option_labels, max_memory):
    ...
    return [
        i
        for i, label in enumerate(option_labels)
        if int(label) == (2 * current + 1)
        or int(label) == (2 * current + 2)
        or int(label) == current
    ]
