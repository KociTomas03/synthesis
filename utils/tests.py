...
class TestPayntRun(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.saynt_output_dir = "test_outpus_saynt"
    ...
    def test_saynt_run(self):
        for project_path in self.projects:
            ...
            for memory_constraint in self.memory_constraints:
                outputFolder = os.path.join(
                    self.saynt_output_dir, project_name, memory_constraint
                )
                output_file = os.path.join(outputFolder, "output.txt")
                output_dir = os.path.dirname(output_file)
                ...
                # Run the paynt.py script as a separate process and capture its output
                command = f"python3 paynt.py {project_path} --fsc-synthesis --storm-pomdp --iterative-storm 900 60 10 --generated-fsc-route {outputFolder}/image --memory-constraint {memory_constraint} > {output_file} 2>&1"
                result = subprocess.run(command, shell=True)
